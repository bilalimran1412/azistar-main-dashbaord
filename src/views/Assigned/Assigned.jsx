...
import CustomerListTooltip from './CustomeListTootip';
...
function Assigned() {
    ...
    const updateMessageStatus = async (customerId, status) => {
        try {
            ...
            if (response.data.success) {
                ...
                SetConfirmationMessage('Failed to update message status.');
            }
        } catch (error) {
        ...
        if (isConfirmed) {
            try {
                ...
                if (response.ok) {
                    ...
                    setActiveClass('success_active')
                    SetConfirmationMessage('Deleting customer successfully');
                } else {
                ...
                setActiveClass('error_active')
                SetConfirmationMessage('Error deleting customer:');
            } finally {
                setLoading(false);
                setTimeout(() => {
                    setActiveClass('');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setActiveClass('success_active')
            SetConfirmationMessage('Customer Update successfully');
        } catch (error) {
                ...
                {selectedCustomer ? (
                    <div>
                        ...
                        <ChatSection>
                                                ...
                                                <Message key={index} sender={msg.sender}>
                                                    <div className={`message-content ${msg.sender === 'system' ? 'system_chat' : ''}`}>
                                                        {msg.sender === 'user' ? (
                                                            ...
                                                            <>
                                                                <div className="customer-icon">
                                                                    <Avatar email='bot' isHovered={hoveredCustomerId === msg.customerId} isActive={selectedCustomer === msg.customerId} />
                                                                </div>
                                                                <span>Bot</span>
                                                                <span className="timestamp">{new Date(msg.timestamp).toLocaleTimeString()}</span>
                                                            </>
                            ...
                            {/* Input section */}
                            <div className="input-section chat_btm">
                                <form onSubmit={e => {
                                    e.preventDefault();
                                    sendMessage();
                                }}>
